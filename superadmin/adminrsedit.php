...
<?php
	...
	$id_admin_cek = false;
	if(isset($_POST['edit'])){
    $id_admin_cek = $_POST['edit'];
 	}
//    echo $id_doctor_cek; //guarenteed to be set so isset not needed

	$query = mysqli_query($conn, "SELECT rsid_admin, admin_name, rsname, email_admin,password_admin
    FROM rsadmin where rsid_admin = '$id_admin_cek'");
	...
	if($data['rsid_admin']){
		$_SESSION['rsid_admin'] = $data['rsid_admin'];
	}
                ...
                <div class="row">
                    ...
                    <div style="width: 80%; margin: auto;">
                  	<table class="table centered">
											<form id="signupForm" class="form-horizontal" action="../access/editproccessadmin.php" method="POST">
												<div class="form-group">
										      <label for="name">Name:</label>
										      <input type="text" class="form-control" maxlength="52" name="name" id="name" placeholder="Enter Name"
										      ...
										      <input type="text" class="form-control" maxlength="52" name="rsname" id="rsname"
													placeholder="Enter Hospital" value="<?php echo $data['rsname'] ?>" required>
                        <hr>
												<div class="form-group">
										      <label for="email">Email:</label>
										      <input type="email" class="form-control disabled" maxlength="36" name="email" id="email"
													placeholder="Enter Email" value="<?php echo $data['email_admin'] ?>" disabled>
													<input type="hidden" name="email2" value="<?php echo $data['email_admin'] ?>">
										    </div>
													<center><button type="submit" class="btn btn-primary" href="adminregistration.html">
														Change Data</button></center>
										  </form>
